...
uint8_t* crr_calc_blake2bp(uint8_t *buf, uint32_t len) {
    uint8_t *crr_blake2bp = (uint8_t *) xzalloc(BLAKE3_OUT_LEN);
    blake2bp_state state;
    blake2bp_init(&state, BLAKE3_OUT_LEN);
    blake2bp_update(&state, buf, len);
    blake2bp_final(&state, crr_blake2bp, BLAKE3_OUT_LEN);
    return crr_blake2bp;
