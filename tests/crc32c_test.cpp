...
/// 1 gigabyte
const uint64_t NumBytes = 1024ULL * 1024ULL * 1024ULL;
...
//   char* data = new char[NumBytes];
  ...
  for (uint64_t i = 0; i + DefaultChunkSize < NumBytes; i += DefaultChunkSize) {
    crc = 0;
    crc = crc32c_isal((const unsigned char*)data + i, DefaultChunkSize, crc);
  }
  duration  = seconds() - startTime;
  ...
  diff = std::chrono::high_resolution_clock::now() - start_1;
  printf("rolling ISA-L hash  :  %.10fs, %.3f MB/s\n",
  ...
  // diff = std::chrono::high_resolution_clock::now() - start_1;
  // printf("rolling Gear Hash    :  %.10fs, %.3f MB/s\n",
  //         diff.count(), (NumBytes / (1024*1024)) / (diff.count()));
